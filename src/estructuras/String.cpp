...
namespace aed
{
    ...
    {
        if (inicio < 0 || inicio >= _longitud || cantidad <= 0)
            return String();
        if (inicio + cantidad > _longitud)
            cantidad = _longitud - inicio;
    ...
    String String::aMinusculas() const
    {
        ...
        for (int i = 0; i < resultado._longitud; ++i)
        {
            ...
            if (c >= 'A' && c <= 'Z')
                resultado._datos[i] = static_cast<char>(c - 'A' + 'a');
        }
        return resultado;
    }
    ...
    {
        if (prefijo._longitud > _longitud)
            return false;
        for (int i = 0; i < prefijo._longitud; ++i)
        {
            if (_datos[i] != prefijo._datos[i])
                return false;
        }
        return true;
    }
    ...
    {
        if (patron._longitud == 0)
            return true;
        if (patron._longitud > _longitud)
            return false;
