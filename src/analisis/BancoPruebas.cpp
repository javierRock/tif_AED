...
namespace aed
{
    ...
    namespace
    {
                                  ...
                                  Arreglo<int> &indices)
        {
            ...
            while (indices.tamanio() < cantidad && intentos < maximoIntentos)
            {
                ...
                if (red.usuarioEnIndice(indice).activo)
    ...
    void BancoPruebas::medirOperaciones(RedSocial &red, Arreglo<ResultadoPrueba> &resultados)
    {
        ...
        {
            if (red.buscarPorCorreo(correos[i]) != nullptr)
                ++encontrados;
        }
        ...
        if (paresPesados * 2 > muestra.tamanio())
            paresPesados = muestra.tamanio() / 2;
        ...
        reloj.iniciar();
        for (int i = 0; i < REPETICIONES_RANKING; ++i)
            red.usuariosMasActivos(10, ranking);
        anotar(resultados, "Top 10 usuarios activos", escala, REPETICIONES_RANKING,
               reloj.milisegundos());

        reloj.iniciar();
        for (int i = 0; i < REPETICIONES_RANKING; ++i)
            red.publicacionesConMasReacciones(10, ranking);
        ...
        if (pares * 2 > muestra.tamanio())
            pares = muestra.tamanio() / 2;
        ...
        for (int i = 0; i < cantidadOrdenar; ++i)
            original.agregar(azar.enteroMenorQue(1000000000));
