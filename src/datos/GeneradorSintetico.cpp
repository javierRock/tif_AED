...
const char* const NOMBRES[] = {
    "Ana",      "Luis",     "Maria",    "Carlos",   "Sofia",    "Jorge",    "Lucia",
    "Miguel",   "Elena",    "Pedro",    "Carmen",   "Diego",    "Rosa",     "Javier",
    "Paula",    "Andres",   "Isabel",   "Fernando", "Marta",    "Ricardo",  "Julia",
    "Alberto",  "Silvia",   "Raul",     "Teresa",   "Oscar",    "Beatriz",  "Hugo",
    "Natalia",  "Sergio",   "Claudia",  "Ivan",     "Patricia", "Manuel",   "Veronica",
    "Daniel",   "Alicia",   "Ruben",    "Monica",   "Pablo",    "Gloria",   "Esteban",
    "Rocio",    "Tomas",    "Adriana",  "Felipe",   "Renata",   "Gabriel",  "Valeria",
    "Ernesto",  "Milagros", "Alonso",   "Fiorella", "Rodrigo",  "Katherine", "Cesar",
    "Yesenia",  "Marco",    "Pilar",    "Vicente",  "Susana",   "Emilio",   "Lorena",
    "Arturo",   "Nadia",    "Ramiro",   "Ximena",   "Gonzalo",  "Camila",   "Ignacio",
    "Roxana",   "Mateo",    "Antonia",  "Bruno",    "Delia",    "Enrique",  "Flavia",
    "Gustavo",  "Helena",   "Israel",   "Jimena",   "Kevin",    "Liliana",  "Martin",
    "Noelia",   "Octavio",  "Priscila", "Quintin",  "Rafaela",  "Samuel",   "Tatiana",
    "Ulises",   "Vania",    "Wilmer",   "Yolanda",  "Zacarias", "Abel",     "Brenda",
    "Cristian", "Dalia",    "Efrain",   "Fabiola",  "German",   "Hilda",    "Ismael",
    "Janeth",   "Leonardo", "Magali",   "Nicolas",  "Olga",     "Percy",    "Regina",
    "Salvador", "Thalia",   "Ubaldo",   "Vladimir", "Wendy",    "Yeltsin",  "Zoila",
    "Alfonso",  "Bertha",   "Clemente", "Dorotea",  "Eduardo",  "Francisca", "Genaro",
    "Herminia", "Isidro"};

const char* const APELLIDOS[] = {
    "Quispe",    "Mamani",    "Flores",    "Vargas",    "Rojas",     "Huaman",
    "Ramos",     "Torres",    "Chavez",    "Salazar",   "Cardenas",  "Paredes",
    "Zevallos",  "Aguilar",   "Cordova",   "Ticona",    "Apaza",     "Condori",
    "Gutierrez", "Medina",    "Pacheco",   "Bustamante", "Delgado",  "Fernandez",
    "Cruz",      "Reyes",     "Valdivia",  "Cabrera",   "Sanchez",   "Ortega",
    "Peralta",   "Benavides", "Escobar",   "Guzman",    "Linares",   "Montoya",
    "Nunez",     "Ordonez",   "Portugal",  "Quiroz",    "Rivera",    "Sotomayor",
    "Tapia",     "Ugarte",    "Velasquez", "Yupanqui",  "Zamora",    "Barrios",
    "Arenas",    "Bejarano",  "Calderon",  "Chirinos",  "Duenas",    "Espinoza",
    "Farfan",    "Gamarra",   "Herrera",   "Ibanez",    "Jimenez",   "Lazo",
    "Llerena",   "Manrique",  "Neyra",     "Olivera",   "Palomino",  "Puma",
    "Rendon",    "Roque",     "Sarmiento", "Sucari",    "Talavera",  "Trujillo",
    "Urquizo",   "Valencia",  "Vilca",     "Yanqui",    "Zapana",    "Acosta",
    "Alvarez",   "Anco",      "Begazo",    "Butron",    "Caceres",   "Carpio",
    "Castillo",  "Ccama",     "Chino",     "Choque",    "Coaguila",  "Colque",
    "Corrales",  "Diaz",      "Duran",     "Enriquez",  "Estrada",   "Figueroa",
    "Gallegos",  "Garcia",    "Gomez",     "Hancco",    "Huanca",    "Idme",
    "Juarez",    "Lopez",     "Luque",     "Machaca",   "Maldonado", "Marquez",
    "Mendoza",   "Molina",    "Morales",   "Munoz",     "Navarro",   "Oviedo",
    "Pena",      "Perez",     "Pinto",     "Ramirez",   "Rivas",     "Romero",
    "Salas",     "Silva",     "Soto",      "Suarez",    "Tejada",    "Valverde",
    "Vera",      "Villanueva"};
...
long long maximoComunDivisor(long long a, long long b) {
    while (b != 0) {
        long long resto = a % b;
        a = b;
        b = resto;
...
long long pasoCoprimo(long long total) {
    if (total <= 2) return 1;
    long long paso = 1 + 2 * (total / 3);
    while (maximoComunDivisor(paso, total) != 1) paso += 2;
    return paso;
}
...
long long GeneradorSintetico::combinacionesDeNombre() {
    return static_cast<long long>(CANTIDAD_NOMBRES) * CANTIDAD_APELLIDOS * CANTIDAD_APELLIDOS;
}

void GeneradorSintetico::generarUsuarios(RedSocial& red, const ParametrosGeneracion& parametros,
                                         GeneradorAleatorio& azar) {
    char bufferCorreo[128];

    long long total = combinacionesDeNombre();
    long long paso = pasoCoprimo(total);
    long long desplazamiento = static_cast<long long>(parametros.semilla % static_cast<unsigned long long>(total));

    for (int i = 0; i < parametros.cantidadUsuarios; ++i) {
        long long combinacion = (desplazamiento + i * paso) % total;

        const char* nombrePila = NOMBRES[combinacion % CANTIDAD_NOMBRES];
        combinacion /= CANTIDAD_NOMBRES;
        const char* apellidoPaterno = APELLIDOS[combinacion % CANTIDAD_APELLIDOS];
        combinacion /= CANTIDAD_APELLIDOS;
        const char* apellidoMaterno = APELLIDOS[combinacion % CANTIDAD_APELLIDOS];

        String nombreCompleto(nombrePila);
        nombreCompleto.agregar(" ");
        nombreCompleto.agregar(apellidoPaterno);
        nombreCompleto.agregar(" ");
        nombreCompleto.agregar(apellidoMaterno);


        std::snprintf(bufferCorreo, sizeof(bufferCorreo), "%s.%s.%s%d@redsocial.pe",
                      nombrePila, apellidoPaterno, apellidoMaterno, i);
